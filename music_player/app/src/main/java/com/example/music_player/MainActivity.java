...
public class MainActivity extends AppCompatActivity {
    ...
    private static final String key_key = "com.example.music_player.palash";
        ...
        Dexter.withContext(this)
                ...
                .withListener(new PermissionListener() {
                    ...
                    public void onPermissionGranted(PermissionGrantedResponse permissionGrantedResponse) {
                        ...
                        ArrayList<File> mySongs = fetch_songs(Environment.getExternalStorageDirectory());
                        String[] items = new String[mySongs.size()];
                        for(int i = 0; i < items.length; i++)
                        {
                            items[i] = mySongs.get(i).getName().replace(".mp3", "");
                        }
                        CustomAdapter ad = new CustomAdapter(MainActivity.this, R.layout.my_custom_lv, items);
    ...
    public ArrayList<File> fetch_songs(File file)
    {
        ...
        if(songs != null)
        {
            for(File myfile: songs)
            {
                ...
                {
                    arrayList.addAll(fetch_songs(myfile));
                }
                else
                {
